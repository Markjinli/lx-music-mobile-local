...
**另有一份私密备份：** 密钥库和密码存在一个独立的 GitHub **私密**仓库里（仓库名与本机备份说明在密钥那份 README 中，不在此文档公开）。

- 该私密仓库**不要改成公开**，不要 fork，不要加协作者
- 存放密钥的账号和本仓库账号都要开启**两步验证**
- 私密仓库只是备份之一：若账号被盗，密钥即泄露，拿到的人可以签出能覆盖安装的假更新包
- 密钥**泄露比丢失更危险**，且安卓没有吊销机制，出事只能换新密钥并让所有用户卸载重装

## 修改 CI 时注意

`.github/workflows/beta-pack.yml` 在推送到 `beta` 分支时也会触发，并且使用同一套签名密钥。**不要往任何会触发 workflow 的分支推不可信的内容** —— 有仓库写权限的人都能改 workflow 把密钥打印出来。
